...
export type EntityIndexResponse = {
    html: {
        index: string,
        pagination: string
    };
...
export default class EntityIndexPage {
    ...
    private init(): void {
        this.content = document.querySelector(".js-index-content");

        if (this.content === null) {
            return;
        }

        this.filter = document.querySelector(".js-entity-filter");
        ...
        try {
            fetch(url, {
                ...
                if (jsonResponse.hasOwnProperty("errors")) {
                    console.error("Error while loading pagination: " + jsonResponse.message);
                    return;
                }

                jsonResponse = jsonResponse as EntityIndexResponse;

                if (reload) {
                    this.content.innerHTML = jsonResponse.html.index;
                } else {
                    this.content.insertAdjacentHTML("beforeend", jsonResponse.html.index);
                }
