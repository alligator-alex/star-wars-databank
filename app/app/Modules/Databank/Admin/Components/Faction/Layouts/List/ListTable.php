...
namespace App\Modules\Databank\Admin\Components\Faction\Layouts\List;
        ...
        return [
            TD::make('id', __('ID'))
                ...
                ->render(static fn (Faction $model): Field => Link::make('# ' . $model->id)
                    ->route(FactionRouteName::ONE->value, $model->id, false)
                    ->class('text-muted'))
                ->width('100px'),
            ...
            TD::make('name', __('Name'))
                ->cantHide()
                ->sort()
                ->render(static fn (Faction $model): View => view('admin.databank.factions.list.name-cell', [
                    'model' => $model,
                ]))
                ->width('300px'),
